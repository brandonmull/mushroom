...
def countFeaturePairs(mushroomDataset, featurePairs, featureValues):
    """Count edible and poisonous occurrences for each feature-value pair combination."""
    data = mushroomDataset.data.original
    result = list()
...
def findMinimalSetOfIdentifyingFeaturePairs(
    mushroomDataset, featurePairs, featureValues, edibilityCount, edibility
):
    ...
    while True:
        topFeaturePair = (countFeaturePairs(mushroomDataset, featurePairs, featureValues)
...
def buildFeatureCrosstab(mushroomDataset, feature1Name, feature2Name, edibilityValues=('e', 'p')):
...
def parseFeatureValueMap(variableInfoText):
...
def plotFeatureCrosstab(mushroomDataset, crossTab, title=None, figsize=(14, 10), showValueLegend=False):
    ...
    if showValueLegend:
        featureMap = parseFeatureValueMap(mushroomDataset.metadata['additional_info']['variable_info'])
        legendTextBlocks = []
...
def printFeatureCrosstab(mushroomDataset, crossTab, showValueLegend=False):
